...
1. Descubra o armazenamento local antes de consultar dados. Prefira, nesta
   ordem, `OPENCODE_DATA_DIR` quando definido, o diretório de dados XDG
   (`${XDG_DATA_HOME:-$HOME/.local/share}/opencode`) e, por último,
   `${HOME}/.opencode`. Procure `opencode.db`, arquivos SQLite e diretórios de
   sessões. Não leia `auth.json` nem qualquer arquivo que contenha credenciais.
2. Quando disponível, use `opencode db path` para localizar o banco e
   ...
   Inspecione o schema antes da consulta. Use `session`, `session_input`,
   `message` e `part` quando existirem; nunca suponha que o schema seja igual
   entre versões.
3. Para cada sessão, extraia:
...
4. Converta timestamps Unix em segundos, milissegundos ou microssegundos de
   ...
   HH:mm`.
5. Ordene por `Data Final` decrescente; em empate, por `Data de Início`
   decrescente.
6. Se uma linha ou um arquivo estiver corrompido, ignore apenas essa sessão.
   ...
   as demais consultas.
7. Se não houver sessões válidas, ou o diretório/banco estiver vazio, responda
